...
RegisterNetEvent('qb-drugs:outlawNotify')
AddEventHandler('qb-drugs:outlawNotify', function(alert)
	...
	while true do
		...
		if GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), -1054.33,-230.88,44.02, true) < 1 then
			...
			if (IsControlJustReleased(1, 51)) then
				...
				Citizen.Wait(20000)
				TriggerServerEvent('qb-drugs:pendrive')
...
-- Usable Item Event:
RegisterNetEvent("qb-drugs:UsableItem")
AddEventHandler("qb-drugs:UsableItem",function()
...
RegisterNetEvent("qb-drugs:meth10g")
AddEventHandler("qb-drugs:meth10g",function()
...
function ChooseDrugMenu()
	...
	TriggerServerEvent("qb-drugs:GetSelectedJob",'meth',0, 1, 1)
	ClearPedTasks(player)
		FreezeEntityPosition(player,false)
end
...
RegisterNetEvent("qb-drugs:BrowseAvailableJobs")
AddEventHandler("qb-drugs:BrowseAvailableJobs",function(spot,drugType,minReward,maxReward)
	...
	if currentID == 100 then
		...
		SelectedID = id
		TriggerEvent("qb-drugs:startMainEvent",id,drugType,minReward,maxReward)
		PlaySoundFrontend(-1, "Mission_Pass_Notify", "DLC_HEISTS_GENERAL_FRONTEND_SOUNDS", 0)
	end
end)

-- Core Mission Part
RegisterNetEvent('qb-drugs:startMainEvent')
AddEventHandler('qb-drugs:startMainEvent', function(id,drugType,minReward,maxReward)
	local Goons = {}
	...
	TriggerServerEvent("qb-drugs:syncJobsData",Config.Jobs)
	Citizen.Wait(500)
	...
	while not JobCompleted and not StopTheJob do
		...
		if Config.Jobs[id].InProgress == true then
			...
			if NearOtherVehicle == true and drugBoxInHand and IsControlJustPressed(0, 38) then
				...
				TriggerServerEvent("qb-drugs:JobReward",minRewardD,maxRewardD,typeDrug)
				drugsTaken = true
				StopTheJob = true
			end
...
function LockpickJobVan(selectedJob)
	...
	if Config.PoliceNotfiyEnabled == true then
		TriggerServerEvent('qb-drugs:DrugJobInProgress',GetEntityCoords(PlayerPedId()),streetName)
	end
...
-- Drug Effects Event:
RegisterNetEvent("qb-drugs:DrugEffects")
AddEventHandler("qb-drugs:DrugEffects", function(k,v)
...
-- Convert Item Event:
RegisterNetEvent("qb-drugs:ConvertProcess")
AddEventHandler("qb-drugs:ConvertProcess", function(k,v)
...
Citizen.CreateThread(function()
	while true do
		TriggerServerEvent("qb-drugs:canSellDrugs")
...
RegisterNetEvent("qb-drugs:canSellDrugs")
AddEventHandler("qb-drugs:canSellDrugs", function(soldAmount)
...
RegisterNetEvent('qb-drugs:OutlawBlipEvent')
AddEventHandler('qb-drugs:OutlawBlipEvent', function(targetCoords)
...
RegisterNetEvent("qb-drugs:syncJobsData")
...
function processing()
	...
	QBCore.Functions.Progressbar("coke-", "Breaking down the meth..", 30000, false, true, {
		...
		FreezeEntityPosition(player, false)
		TriggerServerEvent('qb-drugs:process')
		isProcessing = false
	end, function() -- Cancel
		isProcessing = false
