...
RegisterServerEvent("qb-drugs:syncJobsData")
AddEventHandler("qb-drugs:syncJobsData",function(data)
	TriggerClientEvent("qb-drugs:syncJobsData",-1,data)
end)

-- Server side table, to store cooldown for players:
RegisterServerEvent("qb-drugs:addCooldownToSource")
AddEventHandler("qb-drugs:addCooldownToSource",function(source)
...
RegisterServerEvent("qb-drugs:addConvertingTimer")
AddEventHandler("qb-drugs:addConvertingTimer",function(source,timer)
...
RegisterServerEvent("qb-drugs:addDrugEffectTimer")
AddEventHandler("qb-drugs:addDrugEffectTimer",function(source,timer)
...
QBCore.Functions.CreateUseableItem('meth10g', function(source)
	...
	TriggerClientEvent('qb-drugs:meth10g',source)
	Wait(5000)
	xPlayer.Functions.AddItem('meth1g',10)
end)
RegisterServerEvent("qb-drugs:process")
AddEventHandler("qb-drugs:process",function()
	local xPlayer = QBCore.Functions.GetPlayer(source)
	xPlayer.Functions.AddItem('meth10g',10)
end)
RegisterServerEvent("qb-drugs:pendrive")
AddEventHandler("qb-drugs:pendrive",function()
	local xPlayer = QBCore.Functions.GetPlayer(source)
...
QBCore.Functions.CreateUseableItem('drugitem', function(source)
	...
	if not HasCooldown(GetPlayerIdentifier(source)) then
		if xPlayer.Functions.GetItemByName('phone') ~= nil then
			TriggerClientEvent("qb-drugs:UsableItem",source)
		else
...
RegisterServerEvent("qb-drugs:GetSelectedJob")
AddEventHandler("qb-drugs:GetSelectedJob", function(drugType,BuyPrice,minReward,maxReward)
	local xPlayer = QBCore.Functions.GetPlayer(source)
	local itemLabel = ''
	TriggerEvent("qb-drugs:addCooldownToSource",source)
	TriggerClientEvent("qb-drugs:BrowseAvailableJobs",source, 0, drugType, minReward, maxReward)
	label = "Meth"
end)

-- Server Event for Job Reward:
RegisterServerEvent("qb-drugs:JobReward")
AddEventHandler("qb-drugs:JobReward",function()
	local xPlayer = QBCore.Functions.GetPlayer(source)
	...
	for k,v in pairs(Config.DrugEffects) do 
		QBCore.Functions.CreateUseableItem(v.UsableItem, function(source)
			...
			if not DrugEffect(GetPlayerIdentifier(source)) then
				TriggerEvent("qb-drugs:addDrugEffectTimer",source,v.UsableTime)
				xPlayer.Functions.RemoveItem(v.UsableItem,1)
				TriggerClientEvent("qb-drugs:DrugEffects",source,k,v)
			else
...
RegisterServerEvent('qb-drugs:DrugJobInProgress')
AddEventHandler('qb-drugs:DrugJobInProgress', function(targetCoords, streetName)
	TriggerClientEvent('qb-drugs:outlawNotify', -1,string.format("^0Shots fired and ongoing grand theft auto at ^5%s^0",streetName))
	TriggerClientEvent('qb-drugs:OutlawBlipEvent', -1, targetCoords)
end)

RegisterServerEvent('qb-drugs:DrugSaleInProgress')
AddEventHandler('qb-drugs:DrugSaleInProgress', function(targetCoords, streetName)
	TriggerClientEvent('qb-drugs:outlawNotify', -1,string.format("^0Possible drug sale at ^5%s^0",streetName))
	TriggerClientEvent('qb-drugs:OutlawBlipEvent', -1, targetCoords)
end)

RegisterServerEvent("qb-drugs:sellDrugs")
AddEventHandler("qb-drugs:sellDrugs", function()
	local xPlayer = QBCore.Functions.GetPlayer(source)
...
RegisterServerEvent("qb-drugs:canSellDrugs")
AddEventHandler("qb-drugs:canSellDrugs", function()
